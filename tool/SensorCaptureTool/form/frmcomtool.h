...
private:
    ...
    modbus_t *mModbusCtx;       //modbus CTX pointer.
    frmScrollBar *mfrmScrollBa;
    QVector<double> mSensorXval;
    QVector<double> mSensor1val;
    QVector<double> mSensor2val;
    double mXcount;
    int mCountRecv;
    int mCountSend;
...
private slots:
    ...
    void modbusClean(void);
    int modbusRead(unsigned char* destCh, unsigned short* destSh, int addr, int funcCode,int size);
    int modbusWrite(unsigned char* destCh, unsigned short* destSh, int value,int addr,int funcCode,int size);
